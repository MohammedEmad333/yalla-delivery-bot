...
const WELCOME_MESSAGE =
  ...
  'الطلبات الجديدة بنستقبلها من التطبيق فقط، أما هون بنساعدك بأي استفسار أو دعم.\n\n' +
  'اختار اللي بناسبك:\n' +
  '1️⃣ اطلب من التطبيق 📲\n' +
...
const PRICING_MESSAGE =
  '💰 *أسعار التوصيل*\n\n' +
  'السعر بيتحدد حسب المسافة بين موقع الاستلام وموقع التسليم.\n' +
  `• تقريباً كل ${METERS_PER_SHEKEL} متر = 1 ${CURRENCY}\n` +
  `• الحد الأدنى لأجرة التوصيل: ${MIN_FARE} ${CURRENCY}\n\n` +
  '📲 السعر الفعلي بيظهر لك داخل التطبيق قبل تأكيد الطلب:\n' +
  `${APP_WEB_URL}`;
...
const SUPPORT_MESSAGE =
  '💬 *دعم Yalla Delivery*\n\n' +
  'إذا عندك مشكلة أو استفسار، تواصل معنا مباشرة:\n' +
  `📞 واتساب / اتصال: ${SUPPORT_NUMBER}\n` +
  '🕘 أوقات الدعم: يومياً من 9 صباحاً حتى 11 مساءً.\n\n' +
...
function buildAISystemPrompt() {
  return [
    ...
    'اكتب بالعربية الطبيعية وبنفس أسلوب ولهجة العميل قدر الإمكان، وبنبرة ودودة ومختصرة وواضحة.',
...
async function handleMessage(jid, phone, text, hasMedia = false) {
  ...
  if (hasMedia && !raw) {
    return (
      '📎 وصلني الملف. إذا كان متعلقاً بطلب جديد، أنشئ الطلب من التطبيق أولاً.\n\n' +
  ...
  if (AI_ENABLED && activeProvider()) {
    if (isAIRateLimited(session)) {
      return (
        '⏳ وصلت للحد المؤقت للرسائل الذكية. جرّب مرة ثانية بعد شوي.\n\n' +
        'ولإنشاء طلب مباشرة:\n' +
        APP_WEB_URL
      );
    }
